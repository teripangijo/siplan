...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  ...
  $updateSQL = sprintf("UPDATE tbl_surat SET judul=%s, nosur=%s, namapr=%s, tgl1p='$tgl', thn=%s, idstts=%s, idlay=%s, levp=%s, username=%s WHERE idsur=%s",
                       ...
                       GetSQLValueString($_POST['idlay'], "int"),
  ...
  $Result1 = mysql_query($updateSQL, $conn) or die(mysql_error());
   ...
   $updateGoTo = "hal_surat_keluar.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_conn, $conn);
$query_rssur = sprintf("SELECT * FROM vw_sk WHERE idsur = %s", GetSQLValueString($colname_rssur, "int"));
...
mysql_select_db($database_conn, $conn);
$query_rslay = "SELECT * FROM tbl_layan";
        ...
        <div class="user">
          ...
          <div class="user-info">
            <a data-toggle="collapse" href="#collapseExample" class="username">
              <span>
               <strong>Welcome, <?php echo $row_rspe['username']; ?></strong>
                <b class="caret"></b>
              </span>
            </a>
							      ...
							      <tbody>
							        ...
							        <tr>
							          <th width="21%">Nama Perusahaan</th>
                                        <td width="2%">:</td>
                                        <td colspan="2"><?php echo $row_rssur['namapr']; ?>
                                        <input name="namapr" type="hidden" id="namapr" value="<?php echo $row_rssur['namapr']; ?>"></td>
                                      </tr>
							        <tr>
							          <th>Nomor Surat</th>
                                      <td>:</td>
                                      <td colspan="2"><input name="nosur" type="text" id="nosur" value="<?php echo $row_rssur['nosur']; ?>" size="100"></td>
                                      </tr>
							        <tr>
							          <th>Judul Surat</th>
                                      <td>:</td>
                                      <td colspan="2"><input name="judul" type="text" id="judul" value="<?php echo $row_rssur['judul']; ?>" size="100"></td>
                                      </tr>
							        <tr>
							          <th>Janji Layanan</th>
                                      <td>:</td>
                                      <td colspan="2"><select name="idlay" id="idlay">
...
?>
							          ...
							          <tr>
							          <th><a class="btn btn-info" target="" href="hal_surat_keluar.php"></i> Kembali </a></th>
                                      <td>&nbsp;</td>
                                      <td colspan="2"><input type="submit" name="button" id="button" value="Send">
                                      <input name="idsur" type="hidden" id="idsur" value="<?php echo $row_rssur['idsur']; ?>">
                                      <input type="hidden" name="tgl1p" id="tgl1p" value="<?
                                      $tgl1p=date ('Y-m-d H:i:s');
									  echo $tgl1p;
									  ?>">
                                      <input name="thn" type="hidden" id="thn" value="<?php echo $row_rsthn['thn']; ?>">
                                      <input name="idstts" type="hidden" id="idstts" value="<?php echo $row_rssur['idstts']; ?>">
                                      <input type="hidden" name="levp" id="levp" value="<?
                                      $levp=date ('1');
									  echo $levp;
									  ?>">
                                      <input name="username" type="hidden" id="username" value="<?php echo $row_rssur['username']; ?>"></td>
                                      </tr>
							        </tbody>
						        </table>
                              </div>
