...
$tgl = $_POST['tgld'];
if($_SESSION['MM_UserLevel'] == 'pli')
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $insertSQL = sprintf("UPDATE tbl_tanya SET idjen=%s, idpsn=%s, dtya=%s, tgld='$tgl', idlo=%s WHERE idsul=%s",
					   GetSQLValueString($_POST['idjen'], "int"),
					   GetSQLValueString($_POST['idpsn'], "int"),
					   GetSQLValueString($_POST['dtya'], "text"),
...
mysql_select_db($database_conn, $conn);
$query_rsjns = "SELECT * FROM tbl_jenis";
$rsjns = mysql_query($query_rsjns, $conn) or die(mysql_error());
$row_rsjns = mysql_fetch_assoc($rsjns);
$totalRows_rsjns = mysql_num_rows($rsjns);

mysql_select_db($database_conn, $conn);
$query_rstya = sprintf("SELECT * FROM vw_new WHERE idsul=%s", GetSQLValueString($colname_rstya, "int"));
...
mysql_select_db($database_conn, $conn);
$query_rspsn = "SELECT * FROM tbl_pesan";
$rspsn = mysql_query($query_rspsn, $conn) or die(mysql_error());
$row_rspsn = mysql_fetch_assoc($rspsn);
$totalRows_rspsn = mysql_num_rows($rspsn);

mysql_select_db($database_conn, $conn);
$query_rstja = "SELECT * FROM vw_tuju";
$rstja = mysql_query($query_rstja, $conn) or die(mysql_error());
$row_rstja = mysql_fetch_assoc($rstja);
$totalRows_rstja = mysql_num_rows($rstja);
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="col-md-12">
              <div class="card ">
                <div class="card-header card-header-rose card-header-text">
                  <div class="card-text">
                    <h4 class="card-title"><strong>DISTRIBUSI DATA</strong></h4>
                  </div>
                </div>
                <div class="card-body ">
                   <div class="row">
                                ...
                                <table class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        <tr>
                                            ...
                                            <td width="2%">:</td>
                                            <td width="78%"><?php echo $row_rstya['namapr']; ?></td>
                                        </tr>
										<tr>
                                           <th width="20%">Pertanyaan</th>
                                           <td width="2%">:</td>
                                           <td width="78%"><textarea name="dtya" cols="100" id="dtya"><?php echo $row_rstya['dtya']; ?></textarea></td>
                                         </tr>
                                        <tr>
                                          <th>Tanggal Kirim</th>
                                          <td>:</td>
                                          <td><?php echo $row_rstya['tglt']; ?></td>
                                        </tr>
										<tr>
                                    <th>Jenis Pertanyaan</th>
                                    <td>:</td>
                                    <td><select name="idjen" id="idjen">
                                      <option value="Pilih">Pilih</option>
                                      <?php
do {  
?>
                                      <option value="<?php echo $row_rsjns['idjen']?>"><?php echo $row_rsjns['nmj']?></option>
                                      <?php
} while ($row_rsjns = mysql_fetch_assoc($rsjns));
  $rows = mysql_num_rows($rsjns);
  if($rows > 0) {
      mysql_data_seek($rsjns, 0);
	  $row_rsjns = mysql_fetch_assoc($rsjns);
  }
?>
                                        <a href="Home.php">Home | SiPOLan </a>
                                        </select></td>
										</tr>
                                        <tr>
                                          <th>Tujuan Unit Teknis Terkait</th>
                                          <td>:</td>
                                          <td><select name="idlo" id="idlo">
                                          <option value="Pilih">Pilih</option>
                                          <?php do { 
										  ?>
                                          <option value="<?php echo $row_rstja['idlo']?>"><?php echo $row_rstja['namak']?></option>
                                          <?php
                                          } while ($row_rstja = mysql_fetch_assoc($rstja));
                                          $rows = mysql_num_rows($rstja);
                                          if($rows > 0) {
                                          mysql_data_seek($rstja, 0);
	                                      $row_rstja = mysql_fetch_assoc($rstja);
                                           }
                                           ?>
                                      </tr>
                                      <tr>
                                          ...
                                          <td>:</td>
                                          <td><?php echo $row_rstya['tgld']; ?></td>
                                       </tr>
									  <th><span class="style1"><a href="hal_online.php"class="btn btn-info btn-xs">Kembali</a></span></th>
                                    <td>&nbsp;</td>
                                    <td><input type="submit" name="button" id="button" value="Enter">
                                      <input name="idsul" type="hidden" id="idsul" value="<?php echo $row_rstya['idsul']; ?>">
                                      <input type="hidden" name="tgld" id="tgld" value="<?
                                      $tgld=date ('Y-m-d H:i:s');
									  echo $tgld;
									  ?>">
                                      <input name="thn" type="hidden" id="thn" value="<?php echo $row_rsthn['thn']; ?>">
                                      <input name="username1" type="hidden" id="username1" value="<?php echo $row_rstya['username1']; ?>">
