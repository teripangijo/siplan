...
if (isset($_SERVER['QUERY_STRING'])) {
  $editFormAction .= "?" . htmlentities($_SERVER['QUERY_STRING']);
	...
	$temp = explode(".", $_FILES["file2bc"]["name"]);
	$nama_baru = round(microtime(true)). '.' . end($temp);

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {

		$max_size = 1024 * 2000;
		$filesize =$_FILES['file2bc']['size'];
		$tgl = $_POST['tgl2bc'];
		if($filesize <= $max_size) {
  		$updateSQL = sprintf("UPDATE vw_sk SET file2bc='$nama_baru', nosurbc=%s, tgl2bc='$tgl', levbc=%s, idlo=%s WHERE idsur=%s",
                       GetSQLValueString($_POST['nosurbc'], "text"),
					   GetSQLValueString($_POST['levbc'], "text"),
					   GetSQLValueString($_POST['idlo'], "int"),
                       GetSQLValueString($_POST['idsur'], "int"),
					   GetSQLValueString($_FILES['file2bc']['name'], "text"));

  			mysql_select_db($database_conn, $conn);
  			$Result1 = mysql_query($updateSQL, $conn) or die(mysql_error());
  			if(isset($_FILES['file2bc'])){
  move_uploaded_file($_FILES['file2bc']['tmp_name'],'../file20bc/'.$nama_baru);

  $updateGoTo = "hal_keluar.php";
  if (isset($_SERVER['QUERY_STRING'])) {
            ...
            <div class="col-md-12">
              <div class="card ">
                <div class="card-header card-header-info card-header-text">
                  <div class="card-text">
                    <h4 class="card-title"><strong>EDIT DATA</strong></h4>
                  </div>
                </div>
                <div class="card-body ">
                              ...
                              <table class="table table-bordered table-striped"><tbody><tr><td><table class="table table-bordered table-striped">
                                ...
                                <tbody>
                                  <tr>
                                    ...
                                    <td>&nbsp;</td>
                                    <td colspan="2"><?php echo $row_rspe['nosur']; ?></td>
                                  </tr>
                                  <tr>
                                    <th width="18%">Judul Surat Permohonan</th>
                                    <td width="2%">:</td>
                                    <td colspan="2"><?php echo $row_rspe['judul']; ?></td>
                                  </tr>
                                  <tr>
                                    <th>Nomor Surat Jawaban</th>
                                    <td>:</td>
                                    <td colspan="2"><input name="nosurbc" type="text" id="nosurbc" value="<?php echo $row_rspe['nosurbc']; ?>" size="50"></td>
                                  </tr>
                                  <tr>
                                    <th>Upload File</th>
                                    <td>:</td>
                                    <td width="23%"><input type="file" name="file2bc" id="file2bc"></td>
                                    <td width="57%"><a href="../file20bc/<?php echo $row_rspe['file2bc']; ?>?idsur=<?php echo $row_rspe['idsur']; ?>" target="_blank">File : </a><a href="../file20bc/<?php echo $row_rspe['file2bc']; ?>?idsur=<?php echo $row_rspe['idsur']; ?>" target="_blank"><?php echo $row_rspe['file2bc']; ?></a></td>
                                  </tr>
                                  <tr>
                                    <th><span class="style1"><a href="hal_keluar.php"class="btn btn-warning btn-xs">Kembali</a></span></th>
                                    <td>&nbsp;</td>
                                    ...
                                    <input type="hidden" name="tgl2bc" id="tgl2bc" value="<?
                                      $tgl2bc=date ('Y-m-d H:i:s');
									  echo $tgl2bc;
									  ?>">
                                    <input type="hidden" name="idstts" id="idstts" value="<?
									  ...
									  echo $nama;
									  ?>">
                                    <input type="hidden" name="levbc" id="levbc" value="<?
                                      $levp=date ('2');
									  echo $levp;
									  ?>">
